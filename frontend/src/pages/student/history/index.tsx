...
const STATUS_STYLE: Record<BorrowStatus, { bg: string; border: string; color: string; icon: string }> = {
  PENDING:  { bg: '#2d1f00', border: 'rgba(245,158,11,0.35)',  color: '#fbbf24', icon: 'fa-hourglass-half' },
  APPROVED: { bg: '#0c1f3d', border: 'rgba(59,130,246,0.35)',  color: '#60a5fa', icon: 'fa-circle-check' },
  RETURNED: { bg: '#0a2218', border: 'rgba(16,185,129,0.35)',  color: '#34d399', icon: 'fa-rotate-left' },
  OVERDUE:  { bg: '#311010', border: 'rgba(239,68,68,0.35)',   color: '#ff4d4f', icon: 'fa-triangle-exclamation' },
  REJECTED: { bg: '#1c1c1c', border: 'rgba(255,255,255,0.12)', color: '#9ca3af', icon: 'fa-ban' },
};
              ...
              pagination={{
                ...
                showTotal: (total: number, range: [number, number]) => (
                  <span style={{ fontSize: 12, color: '#6b7280' }}>
                    Hiển thị <b style={{ color: '#9ca3af' }}>{range[0]}–{range[1]}</b> trong tổng số <b style={{ color: '#9ca3af' }}>{total}</b> phiếu
                  </span>
                ),
              }}
            />
